...
public class AnalyticsParam {
    ...
    @QueryParam("interval")
    @Parameter(description = "The time interval when getting histogram data (in milliseconds)",
            example = "600000")
    private long interval;

    @QueryParam("query")
    @Parameter(description = "The Lucene query used to filter data",
    ...
    @QueryParam("type")
    @Parameter(description = "The type of data to retrieve",
            required = true,
            schema = @Schema(allowableValues = {"group_by", "date_histo,count"}))
    private AnalyticsTypeParam type;

    @QueryParam("ranges")
    @Parameter(description = "Ranges allows you to group field's data. Mainly used to group HTTP statuses code with `group_by` queries",
            example = "100:199;200:299;300:399;400:499;500:599")
    private RangesParam ranges;

    @QueryParam("aggs")
    @Parameter(description = "Aggregations are used when doing `date_histo` queries and allows you to group field's data. Mainly used to group HTTP statuses code",
            example = "field:status or avg:response-time;avg:api-response-time")
    private AggregationsParam aggs;

    @QueryParam("order")
    @Parameter(description = "The field used to sort results. Can be asc or desc (prefix with minus '-') ",
